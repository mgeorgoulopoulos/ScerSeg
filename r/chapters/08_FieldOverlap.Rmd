...
CREATE TABLE Continents(Gene TEXT, Continent TEXT);
INSERT INTO Continents(Gene, Continent) SELECT Gene, 'Eurasia' FROM CoexFields WHERE Field = 'B' OR Field = 'C' OR Field = 'D' OR Field = 'G';
INSERT INTO Continents(Gene, Continent) SELECT Gene, 'America' FROM CoexFields WHERE Field = 'A' OR Field = 'F';
INSERT INTO Continents(Gene, Continent) SELECT Gene, 'Antarctica' FROM CoexFields WHERE Field = 'E';
INSERT INTO Continents(Gene, Continent) SELECT Gene, 'Ocean' FROM Loci WHERE Gene NOT IN (SELECT Gene FROM Continents);
```
...
|----------------|-----------|------------|-----------|-----------|
|PromoterFields:A|Eurasia    |62          |0.027      |1.0        |
|PromoterFields:B|Eurasia    |116         |0.051      |0.97       |
|PromoterFields:F|Eurasia    |192         |0.08       |0.89       |
|MotifFields:B   |Eurasia    |82          |0.04       |0.85       |
|----------------|-----------|------------|-----------|-----------|
|PromoterFields:G|America    |218         |0.21       |0.96       |
|PromoterFields:C|America    |120         |0.12       |1.0        |
|PromoterFields:D|America    |160         |0.16       |1.0        |
|----------------|-----------|------------|-----------|-----------|
|PromoterFields:E|Ocean      |98          |0.035      |0.57       |
|MotifFields:A   |Ocean      |55          |0.02       |0.96       |
|MotifFields:C   |Ocean      |76          |0.03       |0.78       |

## Continent statistics

1000 samples of pairwise coexpression score for each continent:

```{r}
continentCoexpression <- read.table(file = '../../Results/ContinentCoexpressionScoreSamples.tsv', sep = '\t', header = TRUE)
boxplot(continentCoexpression, outline=FALSE)
```

## Gene ontology enrichment

"Ocean" (coexpression insulator) cluster has a 2% PAdj for "Metabolit pathways".
